...
fetch("/qualitycontrol/contributions")
...
document
  ...
  .addEventListener("click", function () {
    ...
    checkForNearbyPotholes(contributionId, latitude, longitude, filename, trafficVolume, hasSidewalk, is_oneway);

  });

function checkForNearbyPotholes(contributionId, latitude, longitude, filename, trafficVolume, hasSidewalk, is_oneway) {
  fetch("/qualitycontrol/approved_potholes")
    .then(response => response.json())
    .then(data => {
      const nearbyPothole = data.find(pothole => {
        console.log("distancer");
        const distance = calculateDistance(latitude, longitude, pothole.latitude, pothole.longitude);
        console.log(distance);
        return distance <= 20; // Distance in meters (5M)
      });

      if (nearbyPothole) {
        console.log("NEARBY");
        showDuplicateModal(nearbyPothole, contributionId, latitude, longitude, filename, trafficVolume, hasSidewalk, is_oneway);
      } else {
        console.log("NONEARBY");
    ...
    .catch(error => console.error("Error fetching approved potholes:", error));
}
...
function showDuplicateModal(nearbyPothole, contributionId, latitude, longitude, filename, trafficVolume, hasSidewalk, is_oneway) {
  const duplicateModal = document.getElementById("duplicateModal");
  document.getElementById("duplicate-modal-image").src = `static/frames/${nearbyPothole.filename}`;
  duplicateModal.classList.remove("hidden");

  document.getElementById("confirm-duplicate-button").onclick = function () {
    sendValidationAction(contributionId, latitude, longitude, filename, trafficVolume, hasSidewalk, is_oneway, "approve");
    duplicateModal.classList.add("hidden");
  };

  document.getElementById("disapprove-duplicate-button").onclick = function () {
    openDisapproveModal();
    duplicateModal.classList.add("hidden");
  };
}
